...
internal static class ProfileOverviewModal{
    private static GameObject _overlayRoot;

    public static bool IsOpen => _overlayRoot && _overlayRoot.activeSelf;

    public static void Show(Transform screenRoot, LifetimeStats lifetime, TextMeshProUGUI styleSource){
        if(!screenRoot) return;

        Hide();

        _overlayRoot = BuildOverlay(screenRoot, lifetime, styleSource);
    ...
    private static GameObject BuildOverlay(Transform screenRoot, LifetimeStats lifetime, TextMeshProUGUI styleSource){
        var style = ScrollContentBuilder.ResolveStyle(styleSource);

        var overlay = new GameObject("ProfileOverviewModal", typeof(RectTransform), typeof(Image));
        ...
        dialogRect.anchorMin        = new Vector2(0.18f, 0.12f);
        dialogRect.anchorMax        = new Vector2(0.82f, 0.88f);
        ...
        CreateHeaderRow(dialog.transform, style);
        CreateBody(dialog.transform, lifetime, style);
    ...
    private static void CreateHeaderRow(Transform parent, ScrollContentBuilder.ScrollTextStyle style){
        ...
        ScrollRowBuilder.ConfigureText(
                                       titleText,
                                       LocaleLoader.Format(LocaleKeys.ProfileOverviewTitle),
    ...
    private static void CreateBody(
        Transform parent, LifetimeStats lifetime, ScrollContentBuilder.ScrollTextStyle style
    ){
        ...
        LifetimeStatsRows.AddFullOverview(items, lifetime, style);
